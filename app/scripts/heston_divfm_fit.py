"""Generate synthetic IV surfaces from HestonJ and fit a DIVFM network.

Usage::

    uv run python app/heston_divfm_fit.py

Workflow:
    1. Sample random HestonJ (with DoubleExponential jumps) parameter sets.
    2. For each parameter set, price options on a grid of (moneyness_ttm, ttm)
       to produce one day of IV data.
    3. Train a DIVFMNetwork on the resulting collection of days.
"""
...
if __name__ == "__main__":
    ...
    print(f"\nFirst 10 losses (avg): {np.mean(losses[:10]):.6f}")
    print(f"Last  10 losses (avg): {np.mean(losses[-10:]):.6f}")
