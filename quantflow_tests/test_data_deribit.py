...
async def test_loader_loads_known_options(
...
async def test_loader_skips_option_missing_from_instruments(
    deribit_cli: Deribit, options: list[dict], ref_date: datetime
) -> None:
...
async def test_loader_skips_future_missing_from_instruments(
    deribit_cli: Deribit, futures: list[dict], ref_date: datetime
) -> None:
