...
def test_vasicek_process_mapping() -> None:
    curve = VasicekCurve(rate=0.03, kappa=1.2, theta=0.04, sigma=0.1)
...
def test_vasicek_forward_and_discount_shapes() -> None:
    curve = VasicekCurve(rate=0.03, kappa=0.8, theta=0.05, sigma=0.1)
    ttms = np.array([0.0, 0.5, 1.0, 2.0])
    fwd = curve.instantaneous_forward_rate(ttms)
    df = curve.discount_factor(ttms)
...
def test_vasicek_calibrate_recovers_curve() -> None:
    true_curve = VasicekCurve(rate=0.03, kappa=1.5, theta=0.04, sigma=0.06)
