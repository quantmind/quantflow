...
def test_isotropic_noise_detects_scaled_identity() -> None:
    assert isotropic_noise(np.eye(4) * 0.25) == pytest.approx(0.25)


def test_isotropic_noise_single_observation() -> None:
    assert isotropic_noise(np.array([[0.7]])) == pytest.approx(0.7)


def test_isotropic_noise_rejects_unequal_diagonal() -> None:
    assert isotropic_noise(np.diag([0.1, 0.2, 0.1])) is None


def test_isotropic_noise_rejects_off_diagonal() -> None:
    R = np.eye(3) * 0.1
    R[0, 1] = R[1, 0] = 0.05
    assert isotropic_noise(R) is None


def test_isotropic_noise_rejects_non_positive() -> None:
    assert isotropic_noise(np.zeros((2, 2))) is None


def test_mean_and_cov_unpacking() -> None:
...
def test_filter_shapes_2d() -> None:
...
def test_observations_promoted_to_2d() -> None:
    kf = linear_1d().kalman_filter(np.zeros(10))
    assert kf.y.shape == (1, 10)


def test_predict_step() -> None:
...
def test_variance_reduces_toward_observation() -> None:
    m = LinearGaussianModel(
        F=np.eye(1),
        Q=np.eye(1) * 1e-8,
        H=np.array([1.0]),
...
def test_isotropic_fast_path_matches_dense(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # With a 1d state and isotropic R the update takes the Sherman-Morrison
    # fast path. Forcing isotropic_noise to return None falls back to the dense
    # Cholesky solve; the two paths must agree on the same model.
    y = np.random.default_rng(11).normal(scale=0.01, size=(40, 6))
    model = LinearGaussianModel(
        F=np.array([[0.97]]),
        Q=np.array([[1e-4]]),
        H=np.linspace(0.5, 5.0, 6),
        R=np.eye(6) * 0.001**2,
        mu0=np.zeros(1),
        cov0=np.array([[1e-3]]),
    )
    assert isotropic_noise(model.R) is not None
    fast = model.kalman_filter(y)
    ll_fast = fast.filter()

    monkeypatch.setattr("quantflow.ta.kalman.isotropic_noise", lambda R: None)
    dense = model.kalman_filter(y)
    ll_dense = dense.filter()

    assert ll_fast == pytest.approx(ll_dense)
    for f, d in zip(fast.states, dense.states):
        assert np.allclose(f.mean, d.mean)
        assert np.allclose(f.cov, d.cov)


def test_observation_matrix_shapes_match() -> None:
...
def test_ukf_reduces_to_kalman_filter() -> None:
...
def test_ukf_runs_on_nonlinear_model() -> None:
...
def test_ukf_default_parameters() -> None:
