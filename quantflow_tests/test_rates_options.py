...
def _flat(rate: float) -> NelsonSiegel:
    return NelsonSiegel(beta1=Decimal(str(rate)))
...
def test_calibrate_flat_rate() -> None:
    ...
    fitted = NelsonSiegel().calibrator().calibrate(ttm, rates)
...
def test_options_discounting_joint_calibration() -> None:
    ...
    calibration = OptionsDiscountingCalibration(
        asset_curve=NelsonSiegel().calibrator(),
        quote_curve=NelsonSiegel().calibrator(),
        cp=cp,
        strikes=strikes,
        ttm=ttm,
    )
    asset_curve, quote_curve = calibration.calibrate()
    assert isinstance(asset_curve, NelsonSiegel)
    assert isinstance(quote_curve, NelsonSiegel)
